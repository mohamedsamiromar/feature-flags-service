...
Services fetch by owner-scoped key, so a non-owner sees a 404-shaped
``APIError`` (not a 403) and callers pass ``key`` rather than an instance.
"""
...
class TestArchiveFlag:
    ...
    def test_raises_not_found_for_non_owner(self, flag, other_user):
        with pytest.raises(APIError):
            service.archive_flag(flag.key, other_user)

    def test_returns_the_flag_instance(self, flag, user):
        result = service.archive_flag(flag.key, user)
...
class TestUnarchiveFlag:
    def test_sets_is_archived_false(self, user):
        ...
        service.unarchive_flag(flag.key, user)
        flag.refresh_from_db()
    ...
    def test_raises_not_found_for_non_owner(self, other_user):
        flag = FeatureFlagFactory(is_archived=True)
        with pytest.raises(APIError):
            service.unarchive_flag(flag.key, other_user)

    def test_returns_the_flag_instance(self, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        result = service.unarchive_flag(flag.key, user)
        assert result.pk == flag.pk
...
class TestUpdateFlagBlockedWhenArchived:
    def test_update_flag_still_works_on_active_flag(self, flag, user):
        result = service.update_flag(flag.key, user, name="Updated Name")
    ...
    def test_archive_then_update_via_service_raises(self, flag, user):
        ...
        flag.refresh_from_db()
        with pytest.raises(APIError):
            service.update_flag(flag.key, user, name="Changed")

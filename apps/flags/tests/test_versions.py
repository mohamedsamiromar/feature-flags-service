...
from conftest import FeatureFlagFactory, VariationFactory
...
def _make_flag(user, **kwargs):
    ...
    return service.create_flag(user=user, **kwargs)
...
class TestVersionRecording:
    ...
    def test_update_appends_incrementing_versions(self, user):
        flag = _make_flag(user)
        service.update_flag(flag.key, user, rollout_percentage=50)
        service.update_flag(flag.key, user, rollout_percentage=75)
...
class TestRollbackService:
    def test_restores_prior_config(self, user):
        ...
        service.update_flag(flag.key, user, rollout_percentage=90)
        service.rollback(flag.key, user, version_no=1)
    ...
    def test_unknown_version_raises(self, user):
        ...
        with pytest.raises(APIError):
            service.rollback(flag.key, user, version_no=999)

    def test_archived_flag_raises(self, user):
        flag = _make_flag(user)
        service.archive_flag(flag.key, user)
        flag.refresh_from_db()
        with pytest.raises(APIError):
            service.rollback(flag.key, user, version_no=1)
    ...
    def test_non_owner_raises(self, user, other_user):
        flag = _make_flag(user)
        with pytest.raises(APIError):
            service.rollback(flag.key, other_user, version_no=1)
    ...
    def test_dangling_variation_ref_is_dropped(self, user):
        ...
        service.update_flag(flag.key, user, fallthrough_variation=var)  # v2 snapshots var.id
        service.update_flag(flag.key, user, fallthrough_variation=None)  # v3
        var.delete()
        service.rollback(flag.key, user, version_no=2)
        flag.refresh_from_db()
...
class TestVersionAPI:
    def test_list_returns_versions_newest_first(self, auth_client, user):
        flag = _make_flag(user)
        service.update_flag(flag.key, user, rollout_percentage=50)
    ...
    def test_version_detail(self, auth_client, user):
    ...
    def test_rollback_returns_200_and_restores(self, auth_client, user):
        flag = _make_flag(user, rollout_percentage=10)
        service.update_flag(flag.key, user, rollout_percentage=90)
    ...
    def test_rollback_unknown_version_returns_404(self, auth_client, user):
        flag = _make_flag(user)
        resp = auth_client.post(f"{BASE}/{flag.key}/versions/999/rollback/")
        assert resp.status_code == 404

    def test_rollback_unknown_flag_returns_404(self, auth_client):
        resp = auth_client.post(f"{BASE}/nope/versions/1/rollback/")
        assert resp.status_code == 404

    def test_rollback_another_users_flag_returns_404(self, auth_client):
        other_flag = FeatureFlagFactory()  # different owner
        resp = auth_client.post(f"{BASE}/{other_flag.key}/versions/1/rollback/")
        assert resp.status_code == 404

    def test_rollback_archived_flag_returns_409(self, auth_client, user):
        flag = _make_flag(user)
        service.archive_flag(flag.key, user)
        resp = auth_client.post(f"{BASE}/{flag.key}/versions/1/rollback/")

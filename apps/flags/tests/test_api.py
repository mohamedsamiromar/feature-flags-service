...
class TestArchiveEndpoint:
    def test_archive_returns_200(self, base, auth_client, flag):
        resp = auth_client.post(f"{base}/{flag.key}/archive/")
        assert resp.status_code == 200

    def test_archive_sets_is_archived_in_response(self, base, auth_client, flag):
        resp = auth_client.post(f"{base}/{flag.key}/archive/")
        assert resp.json()["is_archived"] is True

    def test_archive_persists_to_db(self, base, auth_client, flag):
        auth_client.post(f"{base}/{flag.key}/archive/")
        flag.refresh_from_db()
        assert flag.is_archived is True

    def test_archive_already_archived_returns_409(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        resp = auth_client.post(f"{base}/{flag.key}/archive/")
        assert resp.status_code == 409

    def test_archive_nonexistent_flag_returns_404(self, base, auth_client):
        resp = auth_client.post(f"{base}/does-not-exist/archive/")
        assert resp.status_code == 404

    def test_archive_another_users_flag_returns_404(self, base, auth_client):
        other_flag = FeatureFlagFactory()  # different owner
        resp = auth_client.post(f"{base}/{other_flag.key}/archive/")
        assert resp.status_code == 404

    def test_archive_writes_audit_log(self, base, auth_client, flag):
        auth_client.post(f"{base}/{flag.key}/archive/")
        assert AuditLog.objects.filter(
            entity_id=str(flag.pk), action="archive"
        ).exists()

    def test_unauthenticated_archive_returns_401(self, base, api_client, flag):
...
class TestUnarchiveEndpoint:
    def test_unarchive_returns_200(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        resp = auth_client.post(f"{base}/{flag.key}/unarchive/")
        assert resp.status_code == 200

    def test_unarchive_sets_is_archived_false_in_response(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        resp = auth_client.post(f"{base}/{flag.key}/unarchive/")
        assert resp.json()["is_archived"] is False

    def test_unarchive_persists_to_db(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        auth_client.post(f"{base}/{flag.key}/unarchive/")
        flag.refresh_from_db()
        assert flag.is_archived is False

    def test_unarchive_active_flag_returns_409(self, base, auth_client, flag):
        resp = auth_client.post(f"{base}/{flag.key}/unarchive/")
        assert resp.status_code == 409

    def test_unarchive_writes_audit_log(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        auth_client.post(f"{base}/{flag.key}/unarchive/")
        assert AuditLog.objects.filter(
...
class TestListWithArchiveFilter:
    def test_list_excludes_archived_by_default(self, base, auth_client, user):
    ...
    def test_list_includes_archived_with_query_param(self, base, auth_client, user):
        FeatureFlagFactory(owner=user, is_archived=False)
        FeatureFlagFactory(owner=user, is_archived=True)
        resp = auth_client.get(f"{base}/?include_archived=true")
        assert resp.json()["count"] == 2

    def test_is_archived_field_present_in_list_response(self, base, auth_client, flag):
        resp = auth_client.get(f"{base}/")
...
class TestPatchArchivedFlag:
    def test_patch_archived_flag_returns_409(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
    ...
    def test_patch_active_flag_succeeds(self, base, auth_client, flag):
        resp = auth_client.patch(
            f"{base}/{flag.key}/",
            {"name": "Updated"},
            format="json",
    ...
    def test_unarchive_then_patch_succeeds(self, base, auth_client, user):
        flag = FeatureFlagFactory(owner=user, is_archived=True)
        auth_client.post(f"{base}/{flag.key}/unarchive/")
        resp = auth_client.patch(
            f"{base}/{flag.key}/",

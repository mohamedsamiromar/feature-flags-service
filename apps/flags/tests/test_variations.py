...
class TestBooleanVariationAutoCreate:
    def test_boolean_flag_creates_true_false_variations(self, user):
        flag = _service.create_flag(user=user, name="F", key="f1")
    ...
    def test_boolean_flag_sets_off_variation(self, user):
        flag = _service.create_flag(user=user, name="F", key="f2")
        assert flag.off_variation.value is False

    def test_boolean_flag_sets_fallthrough_variation(self, user):
        flag = _service.create_flag(user=user, name="F", key="f3")
...
class TestVariationListCreate:
    def test_list_returns_variations(self, auth_client, flag):
    ...
    def test_create_returns_201(self, auth_client, flag):
    ...
    def test_create_persists_to_db(self, auth_client, flag):
        auth_client.post(
            f"{BASE}/{flag.key}/variations/",
            {"name": "red", "value_type": "string", "value": "red"},
            format="json",
    ...
    def test_list_another_users_flag_returns_404(self, auth_client):
...
class TestVariationDetail:
    def test_patch_updates_value(self, auth_client, flag):
    ...
    def test_delete_returns_204(self, auth_client, flag):
    ...
    def test_delete_removes_from_db(self, auth_client, flag):
        v = VariationFactory(flag=flag, name="v", value_type="boolean", value=True)
        auth_client.delete(f"{BASE}/{flag.key}/variations/{v.id}/")
        assert not Variation.objects.filter(pk=v.pk).exists()

    def test_patch_another_users_variation_returns_404(self, auth_client):
        other_flag = FeatureFlagFactory()
...
class TestFlagTypeField:
    def test_flag_response_includes_flag_type(self, auth_client, flag):
        resp = auth_client.get(f"{BASE}/{flag.key}/")
        assert "flag_type" in resp.json()

    def test_flag_response_includes_variations(self, auth_client, flag):
        resp = auth_client.get(f"{BASE}/{flag.key}/")
        assert "variations" in resp.json()

    def test_create_multivariate_flag(self, auth_client):
        resp = auth_client.post(
            f"{BASE}/",
    ...
    def test_boolean_flag_has_two_auto_variations(self, auth_client):
        resp = auth_client.post(
            f"{BASE}/",
